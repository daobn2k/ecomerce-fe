  ...
  return (
    <Row className={styles.formSearch}>
      <Row className={styles.listFilterSearch}>
        <Form.Item className={styles.rowSearch} name='create_uid'>
          <SelectUser label='Người tạo' placeholder='Chọn người tạo' />
        </Form.Item>
        <Form.Item className={styles.rowSearch} name='recive_uid'>
          <SelectUser label='Người nhận' placeholder='Chọn người nhận' />
        </Form.Item>
...
export const SelectUser = (props) => {
  ...
  const getUser = async () => {
    ...
    if (res?.status && res?.data?.result === 'SUCCESS') {
      const { data = [] } = res?.data;
      setUsers(formatDataListApi(data));
    }
  };
  ...
  return <SelectSearchInput label='' placeholder='' options={users} {...props} />;
};
