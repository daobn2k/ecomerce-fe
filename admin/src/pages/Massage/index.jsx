...
const Massage = () => {
  ...
  const handlerGetPosts = async (data) => {
    setLoading(true);
    const res = await new massageApi().getMassage({ ...data });
  ...
  const onRemove = async (id) => {
    const valuesForm = form.getFieldValue();
    showLoading();
    const res = await new massageApi().remove({ id });
    if (res?.data?.success) {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 100,
      render: (ele, item) => (
        <ActionTable onEdit={() => navigate(`${paths.massage}/${item._id}`)} onRemove={() => onRemove(item?._id)} />
      ),
    },
  ];
    ...
    <>
      <Form className={styles.form} form={form} onValuesChange={onFinish} name='form-search-user'>
        <Row className={styles.Posts}>
          <HeaderSearch title='Danh sách massage' form={form} />
          <TableCustomize
            columns={columns}
            data={data}
