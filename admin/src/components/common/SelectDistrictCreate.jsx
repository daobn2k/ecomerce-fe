...
const SelectDistrictCreate = (props) => {
  ...
  const getOptionsDistrict = async (provinceName) => {
    const res = await new postsApi().getDistrict({ provinceName });
    if (res?.data?.success) {
...
export default SelectDistrictCreate;
